...
func Update(ctx context.Context, c *app.RequestContext) {
	...
	handler.DoWithRepo(c,
		func(req *UpdateRepoReq) string { return req.Key },
		func(repo *po.Repo, req *UpdateRepoReq) (*repoModel.RepoDTO, error) {
			...
			if err := db.NewRepoDAO().Save(repo); err != nil {
				return nil, handler.ErrInternal(err.Error())
			}
			return toProtoRepo(*repo), nil
		},
		...
		func(repo *po.Repo, req *DeleteRepoReq) (map[string]string, error) {
			...
			if syncSvc := syncv2.GetService(); syncSvc.GetCore() != nil {
				if tasks, _ := syncSvc.ListTasks(ctx, repo.Key); len(tasks) > 0 {
					return nil, handler.ErrBadRequest("cannot delete repo used in sync tasks")
				}
			}
...
func ScanDirectory(ctx context.Context, c *app.RequestContext) {
	handler.BindAndDo(c,
		func(req *api.ScanDirectoryReq) (api.ScanDirectoryResp, error) {
			if req.Path == "" {
				return api.ScanDirectoryResp{}, handler.ErrBadRequest("path is required")
			}

			if _, err := os.Stat(req.Path); os.IsNotExist(err) {
				return api.ScanDirectoryResp{}, handler.ErrBadRequest("path does not exist")
			}
...
func BatchCreate(ctx context.Context, c *app.RequestContext) {
	handler.BindAndDo(c,
		func(req *api.BatchCreateRepoReq) (api.BatchCreateRepoResp, error) {
			if len(req.Repos) == 0 {
				return api.BatchCreateRepoResp{}, handler.ErrBadRequest("repos is required")
			}
			...
			return api.BatchCreateRepoResp{
				Success: success,
				Failed:  failed,
			}, nil
		},
...
func Clone(ctx context.Context, c *app.RequestContext) {
	handler.BindAndDo(c,
		func(req *api.CloneRepoReq) (map[string]string, error) {
			gitSvc := git.NewGitService()
			...
			if _, err := os.Stat(req.LocalPath); err == nil {
				if gitSvc.IsGitRepo(req.LocalPath) {
					return nil, handler.ErrBadRequest("directory already contains a git repository")
				}
			}
...
func Fetch(ctx context.Context, c *app.RequestContext) {
	...
	handler.DoWithRepo(c,
		func(req *FetchRepoReq) string { return req.RepoKey },
		func(repo *po.Repo, req *FetchRepoReq) (map[string]string, error) {
			gitSvc := git.NewGitService()
			authSvc := auth.NewAuthService()

			remotes, err := gitSvc.GetRemotes(repo.Path)
			if err != nil {
				return nil, handler.ErrInternal("failed to list remotes: " + err.Error())
			}
			...
			if len(errors) > 0 {
				return nil, handler.ErrInternal(strings.Join(errors, "; "))
			}

			return map[string]string{"message": "fetched"}, nil
		},
	)
}
