<template>
  <div class="compare-page">
    ...
    <div class="control-panel">
      <div class="branch-select-group">
        <div class="branch-box">
          ...
          <el-select v-model="source_branch" placeholder="选择源分支" filterable>
        ...
        <div class="branch-box">
          ...
          <el-select v-model="target_branch" placeholder="选择目标分支" filterable>
            <el-option v-for="b in localBranches" :key="b" :label="b" :value="b" />
          </el-select>
        </div>
      </div>
    ...
    <el-alert
      v-if="target_branch && isRemoteBranch(target_branch)"
      title="目标分支不能是远程分支"
      type="warning"
      :closable="false"
      show-icon
    ...
    <template v-if="compareResult">
      ...
      <div class="file-table-card">
        ...
        <div
          ...
          :key="f.path"
          class="table-row"
          :class="{ active: f.path === current_file }"
      ...
      <div v-if="current_file" class="diff-section">
        <div class="diff-header">
          <span class="diff-title">{{ current_file }}</span>
...
const canMerge = computed(() => {
  return target_branch.value && !isRemoteBranch(target_branch.value)
})
...
onMounted(async () => {
  try {
    const res = await getBranchList(repo_key, { page_size: 1000 })
    allBranches.value = res.list || []
  } catch {}
})

watch(diffViewMode, () => {
  if (current_file.value) selectFile(current_file.value)
})
...
async function handleCompare() {
  if (!source_branch.value || !target_branch.value) {
  ...
  diffHtml.value = ''
  try {
    compareResult.value = await compareBranches(repo_key, source_branch.value, target_branch.value)
  } finally {
...
async function selectFile(path: string) {
  ...
  try {
    const res = await getBranchDiff(repo_key, source_branch.value, target_branch.value, path)
...
async function handleDownloadPatch() {
  try {
    const response = await getBranchPatch(repo_key, source_branch.value, target_branch.value)
    ...
    a.href = url
    a.download = `${source_branch.value}-to-${target_branch.value}.patch`
    a.click()
