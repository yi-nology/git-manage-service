...
## 1. 产品简介
**Git Sync Tool** 是一款专为研发工程师和 DevOps 设计的轻量级 Git 同步管理系统。它解决了多仓库、多分支之间代码同步繁琐、缺乏记录、难以自动化的痛点。通过统一的 Web 界面，用户可以轻松配置同步规则，实现代码在不同远程仓库（如 `origin` 到 `ky`）之间的自动化流转。
