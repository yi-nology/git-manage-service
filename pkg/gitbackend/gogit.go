...
func (b *GoGitBackend) Fetch(ctx context.Context, opts FetchOptions) (*FetchResult, error) {
	...
	authMethod, err := b.buildAuth(opts.Auth)
	if err != nil {
		return nil, err
	}
...
func (b *GoGitBackend) buildAuth(auth AuthConfig) (transport.AuthMethod, error) {
	switch auth.Type {
	case "http_basic":
	...
	case "ssh":
		if auth.SSHKey != "" {
			signer, err := sshgit.NewPublicKeysFromFile("git", auth.SSHKey, "")
			if err == nil {
				return signer, nil
			}
		}
